...
class Bookings extends Controller {
  ...
  function index(){
  	...
  	$this->session->set_userdata('uri', $this->uri->uri_string());
		...
		$layout['showtitle'] = 'Reserve Room';	//$layout['title'];
  ...
  function load(){
	...
	function recurring(){
		...
		foreach($bookings as $booking){
			...
			if ($this->session->userdata('user_id') != 1) {
				$data['date'] = date("Y-n-d", strtotime($booking['date']));
			}
	...
	function accept() {
		...
		$userId = $this->M_bookings->GetUserId($booking_id);
		$emailAd = $this->M_bookings->GetEmailAd($userId->user_id);
		
		if($this->M_bookings->acceptBooking($this->school_id, $booking_id)){
			$msg = $this->load->view('msgbox/info', 'The booking has been <strong>accepted</strong>.', True);
			
			// Send successful email.
			// $this->M_bookings->emailUser($emailAd->email);
			
		} else {
			$msg = $this->load->view('msgbox/error', 'An error occured accepting the booking.', True);
		}
		$this->session->set_flashdata('saved', $msg);
	...
	function reject() {
		...
		$booking_id = $this->uri->segment(3);
		if($this->M_bookings->Cancel($this->school_id, $booking_id)){
			$msg = $this->load->view('msgbox/info', 'The booking has been <strong>rejected</strong>.', True);
		} else {
			$msg = $this->load->view('msgbox/error', 'An error occured rejecting the booking.', True);
		}
		$this->session->set_flashdata('saved', $msg);
		redirect('profile/admin_index');
