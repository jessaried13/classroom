...
class Rooms extends Controller {
  ...
  function info(){
	...
	function index(){
		...
		$body['type'] = $this->M_rooms->GetType(NULL);
		// Set main layout
		$layout['title'] = 'Rooms';
	...
	function add(){
		...
		$body['types'] = $this->crud->Get('types');
	...
	function history($id = NULL) {
		...
		$body['count'] = $this->M_bookings->GetHistoryCount($id);
		$body['rooms'] = $this->M_bookings->GetHistory($id);
		$body['period'] = $this->M_bookings->GetPeriod();
		$body['users'] = $this->M_users->GetAll();
		
		
		$layout['body'] = $this->load->view('rooms/rooms_history', $body, True);
		$this->load->view('index_layout', $layout);
	...
	function roomhistory() {
		...
		$layout['body'] = $this->load->view('rooms/rooms_history_all', $body, True);
		$this->load->view('index_layout', $layout);
    ...
    if ($this->validation->run() == FALSE){
      ...
      // Validation failed
		  ...
		  // Validation succeeded!
			...
			$data['rooms.type_id']		= $this->input->post('type_id');
