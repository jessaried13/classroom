...
class Userauth{
	...
	function GetDepartment($user_id = NULL){
		if($user_id == NULL){ $user_id = $this->object->session->userdata('user_id'); }
		$query_str = "SELECT department_id FROM users WHERE user_id='$user_id' LIMIT 1";
		$query = $this->object->db->query($query_str);
		if($query->num_rows() == 1){
			$row = $query->row();
			$level = $row->department_id;
			return $level;
		} else {
			return false;
		}
	...
	function GetNotificationPing() {
		
		// All "active" bookings (today onwards)
		$query_str = "SELECT * FROM bookings WHERE status=0 AND user_id !=1 ";
		$query = $this->object->db->query($query_str);
	...
	function GetNotificationPingReservation() {
		
		// All "active" bookings (today onwards)
		$query_str = "SELECT * FROM reservations WHERE status=0 AND user_id !=1 ";
		$query = $this->object->db->query($query_str);
