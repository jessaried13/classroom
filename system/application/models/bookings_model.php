...
class Bookings_model extends Model{
  ...
  /**
	 ...
	 */	 	 	 	
	function Get($booking_id = NULL, $school_id = NULL){
		if($school_id == NULL){ $school_id = $this->session->userdata('school_id'); }
		
		// All "active" bookings (today onwards)
		$query_str = "SELECT * FROM bookings WHERE school_id='$school_id' AND status=0 ORDER BY date ASC";
		$query = $this->db->query($query_str);
	...
	function GetNotification($user_id = NULL) {
		...
		// All "active" bookings (today onwards)
		$query_str = "SELECT * FROM bookings WHERE user_id='$user_id' ORDER BY date ASC";
		$query = $this->db->query($query_str);
	...
	function GetHistory($room_id = NULL) {
	...
	function GetHistoryCount($room_id = NULL) {
	...
	function GetUserId($booking_id = NULL) {
		
		$query_str = "SELECT * FROM bookings WHERE booking_id='$booking_id'";
		$query = $this->db->query($query_str);
	...
	function GetEmailAd($user_id = NULL) {
	...
	function emailUser($email_address=NULL)
	{

	$this->email->set_newline("\r\n");
	$this->email->from('seph.gallo@gmail.com'); // change it to yours
	$this->email->to($email_address);// change it to yours
	$this->email->subject('Room Booking Confirmed');
	$this->email->message('Hi. Your room booking has been confirmed.');
	if($this->email->send())
	{
		echo 'Email sent.';
	}
  ...
  function acceptBooking($school_id, $booking_id) {
	
	    $query_str = "UPDATE bookings "
								."SET status=1 WHERE booking_id=$booking_id";
  ...
  function BookingCell($data, $key, $rooms, $users, $room_id, $url){
  	...
  	if(isset($data[$key])){
  		...
  		if(isset($users[$booking->user_id])){
			...
			if(
				...
				if($user_id != $booking->user_id){
					$cancel_msg = 'Are you sure you want to cancel this booking that belongs to ' .$displayname . '?';
				}
		...
		if($this->userauth->CheckAuthLevel(ADMINISTRATOR, $this->authlevel)){
			...
			$cell['body'] = '<a href="'.$book_url.'"><img src="webroot/images/ui/accept.gif" width="16" height="16" alt="Book" title="Book" hspace="4" align="absmiddle" />Book Recurring</a>';
  ...
  function html($school_id = NULL, $display = NULL, $cols = NULL, $date = NULL, $room_id = NULL, $school, $uri = NULL){
  	...
  	if($rooms == False){
		...
		switch($display){
			...
			break;
			case 'day':
				$html .= $this->load->view('bookings/select_date', array('chosen_date' => $date, 'types' => $types), True);
			break;
			default:
  	...
  	$count['days'] = count($school['days_list']);
		...
		// Open form
		$html .= '<form class="scrollStyle" name="bookings" method="POST" action="' . site_url('bookings/recurring') . '">';
					...
					case 'periods':
						...
						*/
						...
						foreach($rooms as $room){
							...
							// Start row
							$html .= '<tr class=filter-'.$room->type_id.'>';
		...
		if($this->userauth->CheckAuthLevel(ADMINISTRATOR, $this->authlevel)){
			$html .= $this->load->view('bookings/make_recurring', array('users' => $school['users']), True);
                    ...
                    $html .= $this->load->view('bookings/make_recurring_date', array('users' => $school['users']), True);
