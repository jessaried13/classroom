...
if($field_id != 'X'){
	$this->load->view('msgbox/warning', 'Changing the type of field now may have adverse effects on existing items.');
}

echo form_open('items/fields/save', array('class' => 'cssform', 'id' => 'fields_add'), array('field_id' => $field_id) );
?>
...
<p>
  ...
  <?php
	...
	<br />
	<?php if($field_id != 'X'){ ?><p class="hint"><span class="error">If you change drop-down list items now, you may have to <strong>re-assign</strong> appropriate values to items.</span></p><?php } ?>
</p>
...
$submit['cancel'] = array('Cancel', '7', 'items/fields');
