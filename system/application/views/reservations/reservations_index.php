...
<div class="reservations_week" style="<?php echo $style ?>;">
<table width="100%" border="0" cellpadding="0" cellspacing="0">
<tr>
<td width="20">
<a href="reservations/view/<?php echo date("Y-m-d", strtotime("yesterday", $chosen_date)); ?>/back"><img src="webroot/images/ui/resultset_previous.gif" width="16" height="16" /></a>
</td>
<td align="center"><?php echo date("l jS F Y", $chosen_date) . ' - ' .$thisweek->name ?></td>
<td width="20">
<a href="reservations/view/<?php echo date("Y-m-d", strtotime("tomorrow", $chosen_date)) ?>/forward"><img src="webroot/images/ui/resultset_next.gif" width="16" height="16" /></a>
</td>
</tr>
</table>
</div>
...
<table border="0" bordercolor="#ffffff" cellpadding="2" cellspacing="2" class="reservations" width="100%">
	...
	foreach($items as $item){
		...
		if($reservations){
			foreach($reservations as $reservation){
				$reservations_items[$reservation['item_id']] = $reservation;
				#echo $reservation['item_id'];
			}
		}
		...
		if($item->photo != NULL){
			$photo = 'webroot/images/itemphotos/640/'.$item->photo;
			$photo_sm = 'webroot/images/itemphotos/160/'.$item->photo;
			$jscript .= "messages[{$item->item_id}] = new Array('$photo_sm','{$item->location}');\n";
		...
		echo "<br /><strong>{$item->name}</strong><br />";
		echo '<span style="font-size:90%">'.$item->username.'&nbsp;</span>';
		echo '</td>';
		...
		foreach($periods as $period){
			if($period->bookable == 1){
				...
				if($days_bitmask->bit_isset($today)){
					...
					if(isset($reservations_items[$item->item_id])){
						if($reservations_items[$item->item_id]['period_id'] == $period->period_id){
							echo $reservations_items[$item->item_id]['reservation_id'];
						}
					}
					#echo $item->item_id;
					echo '</td>';
