  ...
  <?php
	if($types){
  	foreach($types as $type){
	  	$typelist[$type->type_id] = $type->name;		#@field($user->displayname, $user->username);
  	}
  }
	$type_id = @field($this->validation->type_id, $user->type_id, '0');
	echo form_dropdown('type_id', $typelist, $type_id, 'tabindex="'.$t.'"');
	$t++;
	?>
</p>
<?php echo @field($this->validation->type_id_error) ?>

<?php } else {?>

	<div class="alert alert-error">
		<strong>OH NO!</strong> Please add a Room Type first.
	</div>
